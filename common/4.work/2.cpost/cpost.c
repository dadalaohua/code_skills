...
signed char cpostAddHandler(CpostParam *param)
{
    ...
    for (size_t i = 0; i < CPOST_MAX_HANDLER_SIZE; i++)
    {
        if (cposhHandlers[i].handler == NULL)
        {
            ...
            cposhHandlers[i].cur_time = CPOST_GET_TICK();
    ...
    for (size_t i = 0; i < CPOST_MAX_HANDLER_SIZE; i++)
    {
        if (cposhHandlers[i].handler)
        {
            ...
            if (cposhHandlers[i].delay == 0 || 
                (cposhHandlers[i].cur_time < (cposhHandlers[i].cur_time + cposhHandlers[i].delay)
                    ? tick >= (cposhHandlers[i].cur_time + cposhHandlers[i].delay)
                    : (cposhHandlers[i].cur_time > tick && tick > (cposhHandlers[i].cur_time + cposhHandlers[i].delay))))
            {
