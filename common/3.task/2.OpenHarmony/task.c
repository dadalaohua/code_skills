...
static LOS_DL_LIST task_head[XTASK_GROUP_MAX];
...
static int task_enqueue(task_t *task, int task_group)
{
    ...
    LOS_DL_LIST_FOR_EACH_ENTRY_SAFE(task_cur, task_next, &task_head[task_group], task_t, node)
    {
        if (task_cur == task)
        {
    ...
    task->tid = task_tid();
    /* new work to queue */
    LOS_ListTailInsert(&task_head[task_group], &task->node);
...
void task_poll(void)
{
    ...
    for (i = XTASK_GROUP_BASE; i < XTASK_GROUP_MAX; i++)
    {
        if (i == XTASK_GROUP_LOOP || (task_run_flag & (1 << i)))
        {
            ...
            LOS_DL_LIST_FOR_EACH_ENTRY_SAFE(task_cur, task_next, &task_head[i], task_t, node)
            {
