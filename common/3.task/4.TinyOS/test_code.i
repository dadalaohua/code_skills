...
# 44 "/usr/include/i386-linux-gnu/sys/select.h" 3 4
# 1 "/usr/include/time.h" 1 3 4
...
unsigned int test_sem;

volatile unsigned char timers[5];
...
static unsigned char _lc=0; switch(_lc){default:
    while(1)
    {
        ...
        do {test_sem=0;} while(0);;
        do {_lc=(28 +((28%256)==0))%256; return 250 ;} while(0); case (28 +((28%256)==0))%256:;
        do {_lc=(29 +((29%256)==0))%256; return 250 ;} while(0); case (29 +((29%256)==0))%256:;
        do {_lc=(30 +((30%256)==0))%256; return 250 ;} while(0); case (30 +((30%256)==0))%256:;
        do {_lc=(31 +((31%256)==0))%256; return 250 ;} while(0); case (31 +((31%256)==0))%256:;
    }
;}; _lc=0; return 255;
}
...
static unsigned char _lc=0; switch(_lc){default:
    while(1)
    {
        do{ test_sem=1; do {_lc=(42 +((42%256)==0))%256; return 0 ;} while(0); case (42 +((42%256)==0))%256:; if (test_sem>0) return 1;} while(0);;
...
static unsigned char _lc=0; switch(_lc){default:
    while(1)
    {
        ...
        do {_lc=(54 +((54%256)==0))%256; return 250 ;} while(0); case (54 +((54%256)==0))%256:;
        do {_lc=(55 +((55%256)==0))%256; return 250 ;} while(0); case (55 +((55%256)==0))%256:;
    }
;}; _lc=0; return 255;
}
...
void sigroutine(int signo)
{
   switch (signo){
       case 14:
           ...
           {unsigned char i; for(i=5;i>0 ;i--){if((timers[i-1]!=0)&&(timers[i-1]!=255)) timers[i-1]--;}};

           signal(14, sigroutine);
           break;
   }
   return;
}
...
int main(int argc, char* argv[])
{
    ...
    {unsigned char i; for(i=5;i>0 ;i--) timers[i-1]=0; };
    test_sem=0;;
    ...
    for(;;)
    {
        ...
        { if (timers[3]==0) {timers[3]=task3(); continue;} };
    }
