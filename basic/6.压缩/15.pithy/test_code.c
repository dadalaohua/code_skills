...
int file_compress_test_function(void)
{
    ...
    cl = pithy_Compress((char*)testin, src_len, (char*)testout, sizeof(testout), 9);
    if (cl) {
...
int file_decompress_test_function(void)
{
    ...
    cl = pithy_Decompress((char*)testin, src_len, (char*)testout, sizeof(testout));
    if (cl) {
        if(pithy_GetDecompressedLength((char*)testin, src_len, &cl))
            fwrite(testout, 1, cl, fp_dst);
    }
