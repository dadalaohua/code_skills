...
int file_compress_test_function(void)
{
    ...
    lzrw3_compress(COMPRESS_ACTION_IDENTITY, NULL, testin, src_len, testout, (uint32_t *)&identity);
    printf("id:             %d\n", identity->id);
    printf("memory:         %d\n", identity->memory);
    printf("name:           %s\n", identity->name);
    printf("version:        %s\n", identity->version);
    ...
    lzrw3_compress(COMPRESS_ACTION_COMPRESS, memory, testin, src_len, testout, &cl);
    if (cl) {
...
int file_decompress_test_function(void)
{
    ...
    lzrw3_compress(COMPRESS_ACTION_DECOMPRESS, memory, testin, src_len, testout, &cl);
    if (cl) {
        fwrite(testout, 1, cl, fp_dst);
    }
