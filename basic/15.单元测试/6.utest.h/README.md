...
- `--random-order[=<seed>]` will randomize the order that the tests are ran in. If the optional argument is not provided, then a random starting seed is used.
...
Expects that the floating-point values x and y are within epsilon distance of each other.
