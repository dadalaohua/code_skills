...
void *csonDecodeObject(cJSON *json, CsonModel *model, int modelSize)
{
    ...
    for (short i = 0; i < modelSize; i++)
    {
        ...
        {
        case CSON_TYPE_CHAR:
            *(char *)((size_t)obj + model[i].offset) = (char)csonDecodeNumber(json, model[i].key);
            break;
        case CSON_TYPE_SHORT:
            *(short *)((size_t)obj + model[i].offset) = (short)csonDecodeNumber(json, model[i].key);
            break;
        case CSON_TYPE_INT:
            *(int *)((size_t)obj + model[i].offset) = (int)csonDecodeNumber(json, model[i].key);
            break;
        case CSON_TYPE_LONG:
            *(long *)((size_t)obj + model[i].offset) = (long)csonDecodeNumber(json, model[i].key);
            break;
        case CSON_TYPE_FLOAT:
            *(float *)((size_t)obj + model[i].offset) = (float)csonDecodeDouble(json, model[i].key);
            break;
        case CSON_TYPE_DOUBLE:
            *(double *)((size_t)obj + model[i].offset) = csonDecodeDouble(json, model[i].key);
            break;
        case CSON_TYPE_BOOL:
            *(char *)((size_t)obj + model[i].offset) = (char)csonDecodeNumber(json, model[i].key);
            break;
        case CSON_TYPE_STRING:
            *(size_t *)((size_t)obj + model[i].offset) = (size_t)csonDecodeString(json, model[i].key);
            break;
        case CSON_TYPE_LIST:
...
void csonFree(void *obj, CsonModel *model, int modelSize)
{
    ...
    for (short i = 0; i < modelSize; i++)
    {
        ...
        case CSON_TYPE_STRING:
        case CSON_TYPE_JSON:
            cson.free((char *)(*(size_t *)((size_t)obj + model[i].offset)));
            break;
        case CSON_TYPE_LIST:
