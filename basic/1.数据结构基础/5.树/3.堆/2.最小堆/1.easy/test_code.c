...
/************************************************************************/
//最小堆的插入
void insert_min_heap(element *heap, element item) {
    int *total = &((*heap).key);
...
//最小堆的删除
element delete_min_heap(element *heap) {
    int parent, child;
    element temp, item;
    int *total = &((*heap).key);
