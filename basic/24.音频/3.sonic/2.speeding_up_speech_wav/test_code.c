...
int main(int argc, char *argv[])
{
    ...
    for (int i = 2; i < argc; i++) {
        if (strcmp(argv[i], "-o") == 0) {
            ...
            fprintf(stderr,
                    ...
                    "  -s OutSpeed      - 速度（变速不变调）, 只能加速，default: '%0.2f'\n"
                    "  -v OutVolume     - 音量, default: '%0.2f'\n"
                    ,argv[0], Outname, OutSpeed, OutVolume);
            exit(EXIT_FAILURE);
        }
    }
    
    printf("Outname %s OutSpeed %0.2f OutVolume %0.2f\n", Outname, OutSpeed, OutVolume);
    ...
    //sonic_lite 的采样率固定，在 sonic_lite.h 中通过 SONIC_SAMPLE_RATE 修改
    if (OutSampleRate != SONIC_SAMPLE_RATE) {
        fprintf(stderr,
            "sonic_lite only processes wave files with a sample rate of %d Hz.  This file uses %d\n",
            SONIC_SAMPLE_RATE, OutSampleRate);
        exit(1);
    }
    ...
    //申请用于读取文件的buff
    short* pcmbuffer = malloc(sizeof(short) * SONIC_INPUT_SAMPLES);
    //初始化
    sonicInit();
    sonicSetSpeed(OutSpeed);    //速度（变速不变调），sonic_lite 只能加速
    sonicSetVolume(OutVolume);  //音量
    ...
    do {
        //读取数据
        samplesRead = fread(pcmbuffer, sizeof(short), SONIC_INPUT_SAMPLES, wavfp);
        if(samplesRead == 0) {
            sonicFlushStream();
        } else {
            //将数据写入流中以加速或降低速度
            sonicWriteShortToStream(pcmbuffer, samplesRead);
        }
        int samplesWritten;
        do {
            //从流中读取数据
            samplesWritten = sonicReadShortFromStream(pcmbuffer, SONIC_INPUT_SAMPLES);
            if(samplesWritten == 0)
                break;
            
            //写入文件
            fwrite(pcmbuffer, sizeof(short), samplesWritten, newwavfp);
            //统计输出的音频长度
            output_sumsize += (samplesWritten * sizeof(short));
