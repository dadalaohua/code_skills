...
int main(int argc, char *argv[])
{
    ...
    //数据段变为short类型后的长度
    int wavshortlen = wavlen / 2;
    //打印长度
    printf("wavlen %d wavshortlen %d\n", wavlen, wavshortlen);
    
#define IN_LIMIT_LEN 4096
#define OUT_LIMIT_LEN 4096
    
    //申请用于读取文件的buff
    short* pcmbuffer = malloc(sizeof(short) * IN_LIMIT_LEN);
    
    //申请用于输入和输出的buff，用于采样率转换
    float* in = malloc(sizeof(float) * IN_LIMIT_LEN);
    float* out = malloc(sizeof(float) * OUT_LIMIT_LEN);
    ...
    int error = -1;
    //初始化
    //src_state = src_new(SRC_SINC_BEST_QUALITY, channel, &error);      //带宽限制插值器，最高品质
    //src_state = src_new(SRC_SINC_MEDIUM_QUALITY, channel, &error);    //带宽限制插值器，中等品质
    //src_state = src_new(SRC_SINC_FASTEST, channel, &error);           //带宽限制插值器，最快的带宽限制插值器
    src_state = src_new(SRC_ZERO_ORDER_HOLD, channel, &error);        //零阶保持转换器，质量很差，但转换速度非常快
    //src_state = src_new(SRC_LINEAR, channel, &error);                 //线性转换器，质量很差，但转换速度非常快
    if(src_state == NULL) {
        printf("src_state %p\n", src_state);
    ...
    src_data.input_frames  = 0;                         //输入数据的帧数
    src_data.data_out      = out;                       //输出数据
    src_data.output_frames = OUT_LIMIT_LEN / channel;   //输出数据的帧数
    src_data.src_ratio     = (float)OutSampleRate/header.fmt_region.SampleRate;  //输出采样率/输入采样率
    src_data.end_of_input  = 0;                         //如果有更多输入数据可用，则等于0，否则等于1
    
    //清空数据缓存区
    memset(in, 0, sizeof(float) * IN_LIMIT_LEN);
    memset(out, 0, sizeof(float) * OUT_LIMIT_LEN);
    ...
    while(1) {
        //已经消耗掉的帧数
        int consume_len = (IN_LIMIT_LEN / channel) - src_data.input_frames;
        //读取音频文件
        int readlen = fread(pcmbuffer, sizeof(short), consume_len * channel, wavfp);
        if((readlen) != 0) {
            //未消耗的数据挪到前面
            for(int i = 0; i < src_data.input_frames * channel; i++) {
                in[i] = in[i + consume_len * channel];
            }
            //将读取的数据放入输入buff中
            for(int i = 0; i < readlen; i++) {
                in[i + src_data.input_frames * channel] = pcmbuffer[i];
            }
        } else if(src_data.input_frames == 0) { //文件已读完且没有剩余的输入帧数，就结束转换
            break;
        }
        
        src_data.data_in        = in;
        src_data.input_frames   += (readlen / channel); //加上新读取的数据长度
        //printf("readlen: %d input_frames: %ld\n", readlen, src_data.input_frames);
        ...
        //输出长度 = 生成的输出帧数 * 通道数
        int output_short_size = src_data.output_frames_gen * channel;
        //输出的buff
        short* outbuffer;
        ...
        //写入文件
        fwrite(outbuffer, sizeof(short), output_short_size, newwavfp);
        free(outbuffer);
        
        //统计输出的音频长度
        output_sumsize += (output_short_size * sizeof(short));
